...
class Cuztom_Field_Yesno extends Cuztom_Field
{
	function _output( $value )
	{
		...
		$output .= '<div class="cuztom-checkbox-wrap">';
			...
			$output .= sprintf( '<label class="cuztom-label" for="%s_yes">%s</label>', $this->id_name, __( 'Yes', CUZTOM_TEXTDOMAIN ) );
			$output .= '<br />';
			...
			$output .= sprintf( '<label class="cuztom-label" for="%s_no">%s</label>', $this->id_name, __( 'No', CUZTOM_TEXTDOMAIN ) );
		$output .= '</div>';
