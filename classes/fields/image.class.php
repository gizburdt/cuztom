...
class Cuztom_Field_Image extends Cuztom_Field
{
	...
	function _output( $value )
	{
		...
		$output .= sprintf( '<input id="upload-image-button" type="button" class="button js-cuztom-upload" data-cuztom-media-type="image"' . ( ! empty($this->args["preview_size"] ) ? ' data-cuztom-preview-size="' . ( is_array($this->args["preview_size"] ) ? esc_attr( json_encode($this->args["preview_size"] ) ) : $this->args["preview_size"] ) . '"' : '') . ' value="%s" />', __( 'Select image', 'cuztom' ) );
